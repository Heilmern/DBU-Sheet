  ...
  ChangelogEntry(
    ...
    changes: [
      ChangelogChange(ChangeKind.added,
          'Beast Traits: the full Bestial and Monstrous Trait catalogue is in, '
          'granted per-source — Races, Forms, Awakenings and Transformations '
          'that give Beast Traits now offer a picker for each grant (with fixed '
          'and Twinned grants handled automatically), and every pick feeds its '
          'effects into your derived stats.'),
      ChangelogChange(ChangeKind.added,
          'Subraces: the five Races with a Subrace section on the site '
          '(Namekian, Demon, Glass Tribe, Neo-Tuffle, Yardrat) now let you '
          'choose a Subrace, which grants its extra Racial Trait like any '
          'native one.'),
      ChangelogChange(ChangeKind.added,
          'Check for updates: the app checks GitHub Releases on launch (quietly, '
          'surfacing only when a newer build exists) and via a new button on the '
          'home screen, with a dialog that shows the release notes and links out '
          'to the download.'),
      ChangelogChange(ChangeKind.changed,
          'The Racial Life Modifier channel now reaches Max Life, so Beast '
          'Traits and other sources that adjust it flow through correctly.'),
    ],
  ),
  ChangelogEntry(
    version: '0.1.2',
